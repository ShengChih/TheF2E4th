...
type HeaderProps = ComponentProps<'header'> & {
  gotoHexSchoolAnchor?: () => void
  gotoScheduleInfoAnchor?: () => void
}

const Header = ({ className, gotoHexSchoolAnchor, gotoScheduleInfoAnchor }: HeaderProps) => {
  const [, isMobile, ,] = useCheckScreen(deviceWidth)
  return (
    <header
      className={flatClassName({
        common: `z-10 translate-x-0 translate-y-0 w-screen fixed flex overflow-hidden	items-center justify-between bg-[#38241B] h-[62px] px-[40px] ${
          className ?? ''
        }`,
