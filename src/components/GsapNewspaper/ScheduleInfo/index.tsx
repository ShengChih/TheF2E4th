...
const ScheduleInfo: ForwardRefRenderFunction<ScheduleInfoHandle, ScheduleInfoProps> = (
  { children },
  forwardref,
) => {
  const tabletAnimationContainer = useRef<HTMLDivElement>(null)
  const [, isMobile, isTablet] = useCheckScreen(deviceWidth)
        ...
        >
          ...
          >
            ...
            >
              ...
              {ProgressPoints.map(({ starttime, RectangleStyle, ...point }, index: number) => {
                const now = +new Date()
                const rectangleStyle =
                  RectangleStyle + (now >= starttime ? `bg-[#951205]` : `bg-[#3C221B]`)
                return (
                  <SchedulePoint
                    key={`progress-${index}`}
            ...
            >
              ...
              >
                <div
                  className={flatClassName({
                    common: `absolute inset-x-0 mx-auto border-[5px] border-solid ${
                      +new Date() >= +new Date('2022-12-05T00:00:00.000+08:00')
                        ? 'border-[#951205]'
                        : 'border-[#3C221B]'
                    }`,
                    tablet: `md:w-[320px]`,
              ...
              {FinalPoints.map(({ starttime, RectangleStyle, ...point }, index: number) => {
                const now = +new Date()
                const rectangleStyle =
                  RectangleStyle + (now >= starttime ? `bg-[#951205]` : `bg-[#3C221B]`)
                return (
