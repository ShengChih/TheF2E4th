import { memo, useEffect, useState, useCallback, useRef, MouseEvent, FocusEvent, Suspense } from 'react'
...
const SignBox = ({
	...
	const back2SignBox = useCallback((e: MouseEvent) => {
		setMakeSignModule(false)
	}, [setMakeSignModule])

	const switch2MakeSignModule = (e: MouseEvent) => {
	...
	return (
		<div tabIndex={0} ref={boxRef} onBlur={handleBlur} className={flatClassName({
			...
			desktop: `${showMakeSignModule ? 'xl:w-[610px]' : 'xl:w-[343px]'}  xl:p-[16px] xl:gap-y-[12px]`,
		})}>
			{
				!showMakeSignModule && (
					<>
								...
								signBox.map((dataUrl: string, index: number) => {
									return (
										<div key={`sign-record-${index}`} className={flatClassName({
											...
											<div key={`trash-icon-${index}`} className={flatClassName({
