...
const AwardInfoBase: ForwardRefRenderFunction<AwardInfoHandle, AwardInfoProps> = (props, forwardref) => {
	...
	return (
		<section ref={sectionRef}  className={`w-full desktop:h-[879px]`}>
			...
			<div className={`desktop:h-[645px] flex items-center justify-center`}>
				<div className={`grid grid-flow-col desktop:w-[1198px] desktop:h-[461px] desktop:gap-x-[20px]`}>
					<div>
						<div
							ref={teamAwardRef}
							style={{
			...
			<div ref={bottomTextRef}  className={`font-sans font-normal text-center mx-auto text-[#3C221B] desktop:w-[396px] desktop:h-[52px] desktop:leading-[52px] desktop:text-[36px] desktop:mb-[12px]`}>以上皆提供完賽數位獎狀</div>
		</section>
	)
}
