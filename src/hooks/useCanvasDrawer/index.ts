...
const useCanvasDrawer = (
  ...
  }, [worker, offscreen, containerHeight, containerWidth])
  ...
  const handleLoadImage = (inputImage: File, callback?: CallbackFunctionVariadicAnyReturn) => {
    ...
    createImageBitmap(inputImage).then((imgBitmap: ImageBitmap) => {
      if (worker) {
        ...
        try {
          preprocessUploadImage(context, containerWidth, containerHeight, imgBitmap)
          callback && callback(true)
        } catch (error) {
          callback && callback(false)
        } finally {
  ...
  const hanldeRemoveWhiteBg = (
    inputImage: File | Blob,
    callback?: CallbackFunctionVariadicAnyReturn,
  ) => {
    setDrawing(true)
    createImageBitmap(inputImage).then((imgBitmap: ImageBitmap) => {
      if (worker) {
