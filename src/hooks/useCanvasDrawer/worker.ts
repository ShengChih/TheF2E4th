...
self.onmessage = function (e) {
  ...
  try {
    if (e.data.type === CLEAR_ALL) {
    ...
    } else if (e.data.type === INIT_CANVAS && e.data.canvas) {
    ...
    } else if (e.data.type === MOVE_IN_CANVAS && e.data.postion) {
      if (!context) {
        throw Error('get Context failure')
      }
      movePostion(context, e.data.postion)
    } else if (e.data.type === DRAW_IN_CANVAS && e.data.postion && e.data.defaultColor) {
      if (!context) {
        throw Error('get Context failure')
      }
      drawTracking(context, e.data.postion, e.data.defaultColor)
    } else if (e.data.type === UPLOAD_IMAGE_TO_CANVAS && e.data.image) {
      ...
      preprocessUploadImage(context, containerWidth, containerHeight, image)
    } else if (e.data.type === REMOVE_WHITE_BG && e.data.image) {
      const image: Exclude<CanvasImageSource, HTMLOrSVGImageElement> = e.data.image
