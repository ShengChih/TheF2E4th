...
const MakeSignModule = ({
  ...
  useEffect(() => {
    ...
    canvasRef.current && (canvasRef.current.height = canvasHeight)
  ...
  const handleChangeFile = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const { result } = checkFile(e.target.files)
    if (!result) {
      return
    }
  ...
  const changeMode = (e: MouseEvent | KeyboardEvent) => {
  ...
  return (
    <>
      <div className={ModeClassName}>
        <div
          ...
          className={flatClassName({
            common: `flex flex-1 items-center justify-center grow h-full ${
              !isImportMode
                ? 'bg-gradient-to-b from-gnsign-greenl to-gnsign-greenh  text-white'
                : 'bg-white text-gnsign-green'
            }`,
      ...
      <div
        ...
        onKeyDown={handleSelectedFileButton}
        onClick={handleSelectedFileButton}
      >
        <input ref={inputFileRef} type="file" className="hidden" onChange={handleChangeFile} />
        <div
